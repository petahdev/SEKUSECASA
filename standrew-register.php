...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("SELECT email FROM standrew WHERE email = ?");
        ...
        if ($stmt->num_rows > 0) {
            ...
            $sql = "INSERT INTO standrew (first_name, second_name, surname, email, phone_number, course_done, year_of_study, password, image)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if ($insertStmt->execute()) {
                header("Location: st-andrew-success.php"); // Redirect to a success page
                exit;
            } else {
...
if (!empty($errors)) {
    ...
    echo "<script>
            setTimeout(function() {
                window.location.href = 'standrew.php';
            }, 4000);
          </script>";
