...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $sql = "INSERT INTO choir (first_name, second_name, surname, email, phone_number, course_done, year_of_study, password, image)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($insertStmt->execute()) {
            header("Location: choir-success.php"); // Redirect to a success page
            exit;
        } else {
...
if (!empty($errors)) {
    ...
    echo "<script>
            setTimeout(function() {
                window.location.href = 'choir.php';
            }, 4000);
          </script>";
