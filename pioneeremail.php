...
if (isset($_POST['subscribe'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO pioneer (email, phone_number, password) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $email, $phoneNumber, $password);
    if ($stmt->execute()) {
        // Send welcome email
        $welcomeSubject = 'Welcome to SECASA Pioneers!';
        $welcomeBody = "
...
function sendWeeklyReminders() {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $morningSubject = 'Reminder: Pioneer Meeting Today at 7:30 PM';
        $morningBody = "
        <h1>Good Morning!</h1>
        <p>This is a friendly reminder to join us for the choir meeting today at 7:30 PM. Your participation is important for our community.</p>
        <p>We hope to see you there!</p>
        ";
        sendEmail($email, $morningSubject, $morningBody);

        // Send Tuesday evening reminder
        $eveningSubject = 'Final Reminder: Choir Meeting Tonight at 7:30 PM';
        $eveningBody = "
        <h1>Good Evening!</h1>
        <p>This is a final reminder for the choir meeting happening tonight at 7:30 PM. Don't miss out on the opportunity to connect with fellow members.</p>
