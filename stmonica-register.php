...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("SELECT email FROM stmonica WHERE email = ?");
        ...
        if ($stmt->num_rows > 0) {
            ...
            $sql = "INSERT INTO stmonica (first_name, second_name, surname, email, phone_number, course_done, year_of_study, password, image)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if ($insertStmt->execute()) {
                header("Location: st-monica-success.php"); // Redirect to a success page
                exit;
            } else {
...
if (!empty($errors)) {
    ...
    echo "<script>
            setTimeout(function() {
                window.location.href = 'stmonica.php';
            }, 4000);
          </script>";
