...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // If no errors, insert into database (Removed the duplicate email check)
    if (empty($errors)) {
        $hashed_password = password_hash($password, PASSWORD_BCRYPT); // Encrypt password
        $sql = "INSERT INTO pioneer (first_name, second_name, surname, email, phone_number, course_done, year_of_study, password, image)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($insertStmt->execute()) {
            header("Location: pioneer-success.php"); // Redirect to a success page
            exit;
        } else {
...
if (!empty($errors)) {
    ...
    echo "<script>
            setTimeout(function() {
                window.location.href = 'pioneer.php';
            }, 4000);
          </script>";
