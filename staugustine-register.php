...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("SELECT email FROM staugustine WHERE email = ?");
        ...
        if ($stmt->num_rows > 0) {
            ...
            $sql = "INSERT INTO staugustine (first_name, second_name, surname, email, phone_number, course_done, year_of_study, password, image)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if ($insertStmt->execute()) {
                header("Location: st-augustine-sucess.php"); // Redirect to a success page
                exit;
            } else {
...
if (!empty($errors)) {
    ...
    echo "<script>
            setTimeout(function() {
                window.location.href = 'staugustine.php';
            }, 4000);
          </script>";
